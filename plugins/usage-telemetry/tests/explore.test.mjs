...
test('the view round-trips through the URL hash; anything malformed falls back', () => {
  ...
  assert.equal(encodeView(DEFAULT_VIEW), 'rankings', 'defaults stay out of the link; Rankings is the home tab')
  const r = { ...DEFAULT_VIEW, sh: 'tokens', rk: 'speed', cat: 'fix', cm: ['c:claude-opus-5-5', 'r:z-ai/glm-5.3-flash'] }
  assert.deepEqual(decodeView('#' + encodeView(r)), r, 'the rankings choices and compared models round-trip')
  assert.deepEqual(decodeView('#rankings?sh=pie&rk=x&cat=Bad!&cm=q:zz').sh, 'spend')
  ...
  assert.deepEqual([bad.tab, bad.p, bad.m, bad.g, bad.c, bad.t, bad.f, bad.pg, bad.lf], ['rankings', '7', 'spend', ['model', 'skill'], 'bar', 'day', {}, 0, null])
